...
## Zooming in on some interesting COVERAGEa aspects 

### Qs  & $ BY   Q4_UrbRur_Mix // Q9a_d_Mix_comb // Q1_ownership // Cov_pop_rango_lbl
> ... Hystograms / Scatterplot of 
...
Based on the information collected via the survey, we can estimate a % of NRW using the % of water NOT Billed / Total Input water = **`NRWm3_pct`** { = (`Q12b_m3_In_Tot` - `Q13_m3_Fatt_Tot`) / `Q12b_m3_In_Tot` *100 }
...
I also try to explore whether the **% of NRW** shows any type of correlation with some variables that could be relevant: 
...
3. **`Q24_VolLossMech_Has`**  --> most suppliers say "Yes" (have a system to record VOLUME LOSSES) and, as expected, show lower % NRW  
...
4. **`Q39_LeaksDetection_Analyzed`**  --> most suppliers say "Yes" (have a systematic campaign to detect ruptures and LEAKS) and, as expected, show lower % NRW  

<center>
![Plot_BOXNWR_Leaks.png](./output/Plot_BOXNWR_Leaks.png "Plot_BOXNWR_Leaks.png")
</center>

5. **`Q41_ClandConn_Analyzed`**  --> vast majority suppliers say "Yes" (have a system) and, as expected, show lower % NRW (for now the NO sub-sample is too small to further analyze)
