...
### Percent of revenues from tariffs in the composition of OPEX & CAPEX (averages from the Brazil sample) 
```{r tblOPEXCAPEXfromtar, results="asis", warning=FALSE}
load(here::here("output", "tbl_avg_Fin.Rdata"))
# class(tbl_descstat_cov)

caption <-  "Average share of OPEX and CAPEX covered by tariffs - grouped by operator type"
Financials<- pandoc.table.return(tbl_avg_Fin, 
											  keep.line.breaks = TRUE,
											  split.tables = Inf,
											  round = 2, big.mark = ",",
											  justify = "lllrrrrr", caption = caption, style = "simple")
cat(Financials)
